...
let userAuthToken;
const userData = {
  username: 'timezone-create-test-user-01',
  password: '1234',
};
...
test.serial('Anon PATCH /api/v1/timezone', async (t) => {
  const payload = {
    name: 'timezone name',
...
test.serial('User PATCH /api/v1/timezone', async (t) => {
  try {
    await server
      .patch(`/api/v1/timezone/${tz1Id}`)
      .set('x-access-token', userAuthToken)
...
test.serial('Manager PATCH /api/v1/timezone', async (t) => {
  try {
    const managerUser = {
...
test.serial('Admin PATCH /api/v1/timezone', async (t) => {
  try {
    await server
      .patch(`/api/v1/timezone/${tz1Id}`)
      .set('x-access-token', adminAuthToken)
